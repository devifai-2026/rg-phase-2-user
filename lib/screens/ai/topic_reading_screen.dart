...
class _TopicReadingScreenState extends State<TopicReadingScreen> {
  ...
  /// Filled by the form when we have to ask.
  BirthDetails? _birth;
  ...
  void initState() {
    super.initState();
    // Try the saved profile first: most seekers have already given their birth
    // details during onboarding, so they should never be asked twice.
    _fetch();
  }

  Future<void> _fetch({BirthDetails? birth}) async {
  ...
  Future<void> _submitBirth() async {
    if (_birth == null || !_birth!.isComplete) return;
    setState(() => _submitting = true);
    await _fetch(birth: _birth);
  ...
  /// Asked only when the profile has no usable birth data. Saved server-side, so
  /// this appears at most once per seeker.
  Widget _askBirth(RgColors c) {
    final t = L10n.of(context);
    return ListView(
      padding: const EdgeInsets.fromLTRB(16, 20, 16, 32),
      children: [
        Row(children: [
          Icon(widget.icon, color: c.gold, size: 22),
          const SizedBox(width: 10),
          Expanded(
            child: Text(t.birthDetailsNeededFor(widget.title),
                style: TextStyle(color: c.ink, fontSize: 15, fontWeight: FontWeight.w700)),
          ),
        ]),
        const SizedBox(height: 6),
        Text(t.birthDetailsNeededBody, style: TextStyle(color: c.muted, fontSize: 12.5, height: 1.4)),
        const SizedBox(height: 18),
        BirthDetailsForm(
          title: widget.title,
          onChanged: (b) => _birth = b,
        ),
        const SizedBox(height: 20),
        SizedBox(
          width: double.infinity,
          child: ElevatedButton(
            onPressed: _submitting ? null : _submitBirth,
            child: _submitting
